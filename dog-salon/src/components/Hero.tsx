import { PawPrint, CalendarDays, Menu, Phone, MapPin, Sparkles, Heart } from 'lucide-react';
  ...
  <section className="relative min-h-screen flex items-center pt-28 pb-20 overflow-hidden bg-transparent">
    ...
    <header className="absolute top-0 left-0 right-0 z-50">
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-5">
        <div className="flex justify-between items-center bg-white/55 backdrop-blur-xl border border-white/70 shadow-[0_18px_60px_rgba(93,64,55,0.08)] rounded-[28px] px-5 lg:px-7 py-4">
          <div className="flex items-center gap-3 font-black text-2xl text-[#5d4037] tracking-tighter">
          ...
          <button className="md:hidden text-[#5d4037] bg-white/70 rounded-2xl p-2">
            <Menu size={28} />
          </button>
        </div>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-6 lg:px-12 w-full grid lg:grid-cols-12 gap-12 lg:gap-16 items-center relative z-10 mt-8">
      <div className="lg:col-span-7 text-center lg:text-left order-2 lg:order-1">
        ...
        <p className="text-lg md:text-2xl text-[#8d6e63] mb-10 leading-relaxed max-w-2xl mx-auto lg:mx-0 font-medium text-balance">
          Moderní střihy, koupání, péče o srst i šetrná dentální hygiena ultrazvukem bez narkózy.
        ...
        <div className="grid sm:grid-cols-3 gap-3 max-w-2xl mx-auto lg:mx-0">
          ...
          <div className="bg-white/55 backdrop-blur-md border border-white/70 rounded-3xl px-5 py-4 shadow-sm">
            <p className="text-[#f43f5e] font-black text-lg">Ultrazvuk</p>
            <p className="text-[#8d6e63] text-sm font-semibold">dentální hygiena</p>
          </div>
